...
- [ ] 1.3a Add `func updateAttachments(_ attachments: [Attachment], for cipherId: String) async` to the `VaultRepository` protocol and implement it in `VaultRepositoryImpl` (`async` required — `VaultRepositoryImpl` is `@MainActor` and callers run on background tasks) — finds the item by ID and replaces its `attachments` array in-place; `AttachmentRepositoryImpl` injects `VaultRepository` and calls this after a successful upload (appending the new `Attachment`) or delete (removing the deleted ID) so the detail pane updates without a full re-sync, matching the pattern used by `VaultRepositoryImpl.update` and `.create`
...
- [ ] 1b.2 Update `CipherMapper.map(raw:keys:)` to also return the effective cipher key as `Data` — use `raw.key` (EncString, decrypt with vault key) if non-nil, otherwise concatenate `keys.encryptionKey + keys.macKey` → 64-byte `Data`; return as a tuple `(VaultItem, cipherKey: Data)` or equivalent; update ALL existing call sites (`SyncRepositoryImpl`, `VaultRepositoryImpl.update`, `VaultRepositoryImpl.create`) to destructure the new return type — these call sites discard the `cipherKey` value since they do not use it
...
- [ ] 3.4 Implement `delete` — DELETE `/api/ciphers/{id}/attachment/{attachmentId}`; on 200, call `await vaultRepository.updateAttachments(updatedList, for: cipherId)` where `updatedList` is the current attachments array with the deleted ID removed — this patches the in-memory vault cache so the row disappears without a full re-sync
...
- [ ] 4.1 Create `AttachmentMapper` with signature `map(_ dto: AttachmentDTO, cipherKey: CryptoKeys) throws -> Attachment` — takes `CryptoKeys` (not raw `Data`) because `CipherMapper` already holds `keys: CryptoKeys` at call time; reconstructing `CryptoKeys` from concatenated `Data` would be unnecessary; decrypt `fileName` from EncString using `cipherKey`; parse `size` String → Int (throw on failure); map `sizeName` verbatim; set `isUploadIncomplete = (dto.url == nil)`; add `os.Logger(subsystem: "com.prizm", category: "attachments")` — log decryption failures at `.error` and non-numeric size at `.error`; no key material or file content in log output (§V)
