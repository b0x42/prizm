...
- [ ] 1b.4 Implement `VaultKeyServiceImpl` (Data layer) conforming to the Domain `VaultKeyService` protocol: reads from `VaultKeyCache` for the cipher's per-item key; if the cache has no entry for the cipher ID (either no per-item key exists, or cipher was created after last sync), fall back to `crypto.currentKeys().symmetricKey`; if `currentKeys()` throws `PrizmCryptoServiceError.vaultLocked`, translate to `VaultError.vaultLocked` and rethrow — do NOT infer locked state from an empty or missing cache entry, as a missing entry is valid for newly created ciphers
...
- [ ] 6b.2 Create `AttachmentBatchViewModel` (`@Observable`) — accepts `[URL]` from drop handler; builds `[AttachmentBatchItem]` reading file size only (not contents) via each URL's `fileURL`; compute `sizeName` locally using `ByteCountFormatter.string(fromByteCount: Int64(sizeBytes), countStyle: .file)` (server has not yet seen the file at this point); exposes `canConfirm: Bool` (true if ≥1 valid item); exposes `isUploading: Bool` (true while any task is in flight)
...
- [ ] 7.2a Define `TempFileManaging` protocol in Domain layer (`Domain/`) with `func register(url: URL)` and `func cleanup()`; `AttachmentTempFileManager` conforms to it; `AttachmentRowViewModel` depends on `any TempFileManaging` — never the concrete App-layer type
